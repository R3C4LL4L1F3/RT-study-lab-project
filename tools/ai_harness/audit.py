...
def build_audit_record(*, task: dict[str, Any], input_hash: str, rule_results: list[dict[str, str]], recommendation: dict[str, Any], deterministic_hash: str, recheck_passed: bool, recheck_problems: list[str]) -> dict[str, Any]:
    findings_hash = sha256_hex(rule_results)
    run_id = "AIH-RUN-" + sha256_hex({"input_sha256": input_hash, "policy_profile": "RTSL-AIH-V0-POLICY-1", "schema_version": "1"})[:16].upper()
    record = {
