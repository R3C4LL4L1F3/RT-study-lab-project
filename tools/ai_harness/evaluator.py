...
def evaluate_snapshot(
    ...
    *,
    repo_root: Path,
    corrupt_candidate_for_test: bool = False,
    governance_profile: str | None = None,
) -> dict[str, Any]:
    config = load_config(repo_root)
    if governance_profile is not None and governance_profile != config.autonomy.get("profile_id"):
        raise SchemaError(f"unknown governance profile: {governance_profile}")
    task = validate_task(deepcopy(raw), config)
    input_hash = sha256_hex(task)
    rules = evaluate_rules(task, config, governance_profile=governance_profile)
    ...
    fresh_rules = evaluate_rules(task, config, governance_profile=governance_profile)
    ...
    if governance_profile is not None:
        deterministic_payload["governance_profile"] = governance_profile
    ...
    if governance_profile is not None:
        result["governance_profile"] = governance_profile
    return result
