...
class ApprovalTests(unittest.TestCase):
    ...
    def test_QA_003_proposed_to_approved_requires_master_approval(self):
        t=base_task(); t['work_state']='PROPOSED'; t['requested_transition']='APPROVED'; t['approvals']=[]
        r=evaluate_snapshot(t,repo_root=ROOT)
        r054=[f for f in r['deterministic_findings'] if f['rule_id']=='AIH-V0-R054'][0]
        self.assertEqual(r054['status'],'FAIL'); self.assertFalse(r['recommendation']['requested_transition_permitted'])

    def test_QA_003_proposed_to_approved_accepts_authorized_master_approval(self):
        t=base_task(); t['work_state']='PROPOSED'; t['requested_transition']='APPROVED'; t['approvals']=[{'approval_id':'APR-MASTER-1','approval_type':'MASTER','decision':'APPROVED','actor':{'actor_id':'H-MASTER-1','actor_type':'HUMAN','authority_role':'MASTER_PROJECT_CONTROL'},'scope':'PROPOSED->APPROVED','evidence_ref':'MASTER-DECISION-1','timestamp':'2026-08-10T19:00:00Z'}]
        r=evaluate_snapshot(t,repo_root=ROOT)
        self.assertEqual(r['deterministic_status'],'VALID'); self.assertTrue(r['recommendation']['requested_transition_permitted'])
