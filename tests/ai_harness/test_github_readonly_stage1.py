...
def req(op, subject="1", ref=None, path=None):
    return EvidenceRequest(PROJECT_CONTROL_ALIAS, op, subject, ref=ref, path=path)

def provider(*responses, manifest=READ, headers=None):
    t = FixtureTransport(responses)
    p = GitHubReadOnlyEvidenceProvider(StaticCredentialSource(manifest, headers=headers), t)
    return p, t

def with_repo(*responses):
    return (TransportResponse(200, REPO), *responses)


class FrozenGoldenContract(unittest.TestCase):
    def test_GH_001_repository_identity_verified(self):
        p,_=provider(*with_repo())
        self.assertEqual(p.retrieve(req(Operation.REPO_GET)).evidence_state, EvidenceState.VERIFIED)
    ...
    def test_GH_003_repository_identity_mismatch_is_contradictory(self):
        bad={**REPO,"id":999}
        p,_=provider(TransportResponse(200,bad))
        r=p.retrieve(req(Operation.REPO_GET))
        self.assertEqual(r.evidence_state,EvidenceState.CONTRADICTORY)
        self.assertEqual(r.repository_id,999)

    def test_GH_004_commit_found_verified(self):
        p,_=provider(*with_repo(TransportResponse(200,{"sha":"a"*40})))
        self.assertEqual(p.retrieve(req(Operation.COMMIT_GET,"a"*40,ref="a"*40)).evidence_state,EvidenceState.VERIFIED)

    def test_GH_005_commit_absent_missing_only_after_preflight(self):
        p,_=provider(*with_repo(TransportResponse(404,endpoint_supports_absence=True)))
        self.assertEqual(p.retrieve(req(Operation.COMMIT_GET,"deadbeef")).evidence_state,EvidenceState.MISSING)

    def test_GH_006_permission_failure_unverified(self):
        p,t=provider(*with_repo(),manifest={"metadata":"read","contents":"none","pull_requests":"read","checks":"read"})
        r=p.retrieve(req(Operation.REPO_GET))
        self.assertEqual(r.evidence_state,EvidenceState.UNVERIFIED); self.assertEqual(t.calls,[])

    def test_GH_007_explicit_file_verified(self):
        path="docs/ai-harness/x.md"
        p,_=provider(*with_repo(TransportResponse(200,{"path":path,"sha":"b"*40})))
        self.assertEqual(p.retrieve(req(Operation.CONTENT_GET,"file",ref="a"*40,path=path)).evidence_state,EvidenceState.VERIFIED)
    ...
    def test_GH_009_current_pr_head_verified(self):
        sha="c"*40
        payload={"number":22,"state":"open","head":{"sha":sha},"base":{"ref":"main"}}
        p,_=provider(*with_repo(TransportResponse(200,payload)))
        self.assertEqual(p.retrieve(req(Operation.PR_GET,"22",ref=sha)).evidence_state,EvidenceState.VERIFIED)

    def test_GH_010_changed_pr_head_is_stale(self):
        payload={"number":22,"state":"open","head":{"sha":"d"*40},"base":{"ref":"main"}}
        p,_=provider(*with_repo(TransportResponse(200,payload)))
        self.assertEqual(p.retrieve(req(Operation.PR_GET,"22",ref="c"*40)).evidence_state,EvidenceState.STALE)

    def test_GH_011_pr_files_current_complete(self):
        p,_=provider(*with_repo(TransportResponse(200,[{"filename":"x.py","status":"modified"}])))
        r=p.retrieve(req(Operation.PR_FILES_LIST,"22"))
        self.assertEqual((r.evidence_state,r.collection_completeness),(EvidenceState.VERIFIED,CollectionCompleteness.COMPLETE))

    def test_GH_012_pr_commit_membership_current(self):
        sha="e"*40
        p,_=provider(*with_repo(TransportResponse(200,[{"sha":sha}])))
    ...
    def test_GH_018_checks_unavailable_unverified(self):
        p,_=provider(*with_repo(TransportResponse(503,{})))
        self.assertEqual(p.retrieve(req(Operation.CHECK_RUNS_LIST,"sha")).evidence_state,EvidenceState.UNVERIFIED)

    def test_GH_019_merged_state_read(self):
        p,_=provider(*with_repo(TransportResponse(204)))
        self.assertEqual(p.retrieve(req(Operation.PR_MERGED_CHECK,"22")).payload,{"merged":True})
    ...
    def test_GH_020_provider_has_no_write_methods(self):
        ...
        self.assertTrue({"create","update","delete","merge","comment","push"}.isdisjoint(names))
    ...
    def test_GH_024_absent_credential_unverified(self):
        p,t=provider(*with_repo(),manifest=None)
        self.assertEqual(p.retrieve(req(Operation.REPO_GET)).credential_capability,CredentialCapability.NOT_CONFIGURED)
        self.assertEqual(t.calls,[])

    def test_GH_025_secret_never_serialized(self):
        p,t=provider(*with_repo(),headers={"Authorization":"Bearer TOPSECRET"})
        r=p.retrieve(req(Operation.REPO_GET))
        self.assertNotIn("TOPSECRET",repr((r,t.calls)))

    def test_GH_026_provider_absent_offline_v0_boundary(self):
    ...
    def test_GH_030_final_policy_recheck_integration_is_separate_module(self):
        import tools.ai_harness.github_readonly.integration as integration
        self.assertTrue(callable(integration.evaluate_snapshot_with_evidence))

    def test_GH_031_ambiguous_404_unverified(self):
        p,_=provider(*with_repo(TransportResponse(404,endpoint_supports_absence=False)))
        self.assertEqual(p.retrieve(req(Operation.CONTENT_GET,"file",ref="a"*40,path="docs/ai-harness/x.md")).evidence_state,EvidenceState.UNVERIFIED)

    def test_GH_032_authoritative_404_missing_derived_by_provider(self):
        p,_=provider(*with_repo(TransportResponse(404,endpoint_supports_absence=True)))
        self.assertEqual(p.retrieve(req(Operation.CONTENT_GET,"file",ref="a"*40,path="docs/ai-harness/x.md")).evidence_state,EvidenceState.MISSING)

    def test_GH_033_required_permission_none_is_not_verified_read_only(self):
        perms={"metadata":"read","contents":"none","pull_requests":"read","checks":"read"}
        self.assertEqual(validate_permission_manifest(perms),CredentialCapability.UNVERIFIED)

    def test_GH_034_write_permission_is_overprivileged(self):
    ...
    def test_GH_035_precedence_contradictory_highest(self):
        self.assertEqual(resolve_evidence_state(list(EvidenceState)),EvidenceState.CONTRADICTORY)

    def test_GH_036_precedence_unverified_over_stale_missing_verified(self):
        self.assertEqual(resolve_evidence_state([EvidenceState.VERIFIED,EvidenceState.MISSING,EvidenceState.STALE,EvidenceState.UNVERIFIED]),EvidenceState.UNVERIFIED)

    def test_GH_037_precedence_stale_over_missing_verified(self):
        self.assertEqual(resolve_evidence_state([EvidenceState.VERIFIED,EvidenceState.MISSING,EvidenceState.STALE]),EvidenceState.STALE)

    def test_GH_038_incomplete_collection_cannot_claim_exhaustive(self):
        p,_=provider(*with_repo(TransportResponse(200,[{"filename":"x","status":"modified"}],next_page=2),TransportResponse(500,{})))
        r=p.retrieve(req(Operation.PR_FILES_LIST,"22"))
    ...
    def test_GH_040_exhausted_error_free_pagination_complete_not_qa_pass(self):
        p,_=provider(*with_repo(
            TransportResponse(200,[{"filename":"a","status":"modified"}],next_page=2),
            TransportResponse(200,[{"filename":"b","status":"added"}],next_page=None)))
        r=p.retrieve(req(Operation.PR_FILES_LIST,"22"))
        self.assertEqual(r.collection_completeness,CollectionCompleteness.COMPLETE)
        self.assertTrue(r.payload["exhaustive"])
        self.assertNotIn("qa_pass",repr(r).lower())


class AdditionalStructuralTests(unittest.TestCase):
    def test_S1_STRUCT_001_required_operation_set_is_nine(self):
        from tools.ai_harness.github_readonly.provider import ALLOWED_OPERATIONS
    ...
    def test_S1_STRUCT_003_invalid_scalar_payload_unverified(self):
        p,_=provider(*with_repo(TransportResponse(200,{"number":22})))
        self.assertEqual(p.retrieve(req(Operation.PR_GET,"22")).evidence_state,EvidenceState.UNVERIFIED)
