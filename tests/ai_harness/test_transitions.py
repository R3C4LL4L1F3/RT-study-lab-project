...
class TransitionTests(unittest.TestCase):
    ...
    def test_paused_resume_previous_state(self):
        task=base_task(); task.update({'work_state':'PAUSED','pause':{'reason':'q','resume_condition':'r','previous_state':'READY'}})
        self.assertTrue(resume_target_valid(task,'READY')); self.assertFalse(resume_target_valid(task,'IN_VALIDATION'))
